...
def count_tokens(text, model=None):
    if not text:
        return 0
    # Simple estimation: ~3 chars per token for Chinese, ~4 for English
    # Good enough for threshold comparisons in PageIndex
    return len(text) // 3
    ...
    for i in range(max_retries):
        try:
            ...
            if return_finish_reason:
                finish_reason = "max_output_reached" if data["choices"][0].get("finish_reason") == "length" else "finished"
...
async def llm_acompletion(model, prompt):
    # Wrap synchronous llm_completion with asyncio.to_thread
